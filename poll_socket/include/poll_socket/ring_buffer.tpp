...
size_t RingBuffer<N>::get_space() {
    return is_write_wrapped()
        ? read_pos_ - write_pos_ - 1
...
void RingBuffer<N>::advance_write_pos(size_t amount) {
    if (amount >= N) throw std::out_of_range("amount must be less than buf size");
    ...
    if (wrappedWrite && !is_write_wrapped()) throw std::out_of_range("Wrapped write attempted to become >= read position");
    return;
}
...
void RingBuffer<N>::advance_read_pos(size_t amount) {
    ...
    read_pos_ += amount;
    if (read_pos_ >= N) {
        if (!wrappedWrite) throw std::out_of_range("read attempted to pass write");
        wrappedWrite = false;
        read_pos_ %= N;
    }
    // if we wrapped past write function and wrapped bool will disagree
    if (!wrappedWrite && is_write_wrapped()) throw std::out_of_range("read attempted to pass write");
    return;
}
